/***************************************************************************************
 ...
 Program Name: Inheritance Assignment
 
 Date: March 7, 2022
 
 Description: This is a character creation GUI program that allows the user to add, edit, delete, and 
 view characters of type vampire, witch, and vampire-werewolf. The characters' data gets saved to a 
 text file once the user closes the program.
 
 Add: CharacterCreationGUI.java line 577
 Edit: CharacterCreationGUI.java line 671, 423
 Delete: CharacterCreationGUI.java line 905, 980
 View: CharacterCreationGUI.java line 671, 723
 Text File: CharacterCreationGUI.java line 990, 1041
 Inheritance: Character.java, Vampire.java 
 Overload: Vampire.java line 7, 12
 Override: Character.java line 51, Vampire.java line 30
 Array of Records: CharacterCreationGUI.java line 585
 Creation and Usage of Objects: Character.java, Vampire.java, Witch.java, VampireWerewolf.java
...
public class CharacterCreationGUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CharacterCreationGUI frame = new CharacterCreationGUI();
	...
	public CharacterCreationGUI() {
		...
		titleLabel.setBounds(177, 6, 117, 16);
		panel.add(titleLabel);

		deleteButton = new JButton("Delete");
		deleteButton.setBounds(248, 28, 83, 29);
		panel.add(deleteButton);
		deleteButton.setVisible(false);

		editButton = new JButton("Edit");
		editButton.setBounds(139, 28, 83, 29);
		panel.add(editButton);
		editButton.setVisible(false);

		viewButton = new JButton("View");
		viewButton.setBounds(361, 28, 83, 29);
		panel.add(viewButton);
		viewButton.setVisible(false);

		saveButton = new JButton("Save");
		saveButton.setBounds(98, 385, 117, 29);
		panel.add(saveButton);
		saveButton.setVisible(false); // set save button to be invisible at program launch

		cancelButton = new JButton("Cancel");
		cancelButton.setBounds(248, 385, 117, 29);
		...
		genderComboBox.setBounds(177, 190, 145, 27);
		...
		genderLabel.setBounds(33, 194, 130, 16);
		...
		eyeColourLabel.setBounds(33, 237, 130, 16);
		...
		eyeColourComboBox.setBounds(177, 233, 145, 27);
		...
		powersLabel.setBounds(33, 282, 95, 16);
		...
		powersComboBox.setBounds(177, 278, 145, 27);
		...
		spellsLabel.setBounds(33, 282, 130, 16);
		...
		furColourComboBox.setBounds(177, 321, 145, 27);
		...
		nameNoteLabel.setBounds(105, 165, 189, 16);
		...
		nameTextField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if (charType.equals("Witch")) // if user selects witch
				{ 
					displayOptions(3); // call method to display witch options
				}
				if (charType.equals("Vampire-Werewolf hybrid")) 
				{
		...
		// when user clicks the red X button, write to file and close the program
	...
	static void addCharacter(int charType) 
	{
		System.out.println("addCharacter method, clc: " + clc);	
		System.out.println("charCount: "+charCount);

		if (charType == 1) // if user creates a vampire character
		{
		...
		else if (charType == 3) // if user creates a vampire-werewolf character
		{
			...
			vampireWerewolves[indexVampWere] = vampireWerewolf; // add vampire-werewolf object pointer to array
...
//		System.out.println("end of addCharacter method\n");
	}
	
	// overload method for readFromFile method, passing in Character object as a parameter
	static void addCharacter(int charType, Character newChar) 
	{
		if (charType == 1) // if user creates a vampire character
		{
		...
		else if (charType == 3) // if user creates a vampire-werewolf character
		{
			...
			charListComboList[clc] = vampireWerewolves[indexVampWere].getName();
			indexVampWere++;     // increment index value of array by one
			//clc++; // increment character count by one
		}
		clc++;
		charCount++;
	}
	...
	static void updateCharList() 
	{
		...
		//System.out.println("updateCharList method, charListComboList at 0: " + charListComboList[0]);
		//System.out.println("updateCharList method, charListComboList at 1: " + charListComboList[1]);
//		for (int i = 0; i < clc; i++)
//		{
//			System.out.println("character at 0 combo list: " + charListComboList[i]);
//		}
...
//			System.out.println ("adding item:"+charListComboList[i]+" to list");
		}
		charListComboBox.setSelectedIndex(0);
		//charChoice = (String) charListComboBox.getSelectedItem();
//		System.out.println("end of updateCharList method\n");
		// charListComboBox = new JComboBox(charListComboList);
		// charListComboBox.setVisible(true);
	}
	
	// method to find character that user wants to edit
	static int findCharacter(String name) 
	{
		...
		{
			storedName =(String)(vampires[i].getName()); // get name of current vampire object
			System.out.println("findCharacter method, name: " + name + " i: "+i);
			if (name.equals(storedName)) // if current name in vampire array matches character that user wants to edit
			{
				displayOptions(1);     // display base attributes
				displayOptions(2);     // display powers
				displayAttributesV(i); // display saved attributes for vampire character
				charType = "Vampire";  // indicate character type for save button action

				editInd = i; // return index value of vampire that user wants to edit
			}
		}

		for (int j = 0; j < indexWitch; j++) // loop through witch object array
		{
			storedName = (String)(witches[j].getName()); // get name of current witch object
			if (name.equals(storedName)) // if current name in witch array matches
				// character that user wants to edit
			{
				displayOptions(1);
				displayOptions(3);     // display witch options
				displayAttributesW(j); // display saved witch attributes
				charType = "Witch";
				editInd = j; 
			}
		}
		...
		{
			storedName = (String)(vampireWerewolves[k].getName()); // get name of current vampire-werewolf object
			if (name.equals(storedName)) // if current name in vampire-witch array matches character that user wants to edit
			{
				displayOptions(1);
				displayOptions(4); // display vampire-werewolf options
				//displayAttributesV(k);
				displayAttributesVW(k); // display saved vampire werewolf attributes
				charType = "Vampire-Werewolf hybrid";
				editInd = k;
			}
		}
		//System.out.println("end of findCharacter method\n");
	...
	{
		//System.out.println("displayAttributesV method, index: " + index);
		...
		//System.out.println("end of displayAttributes method\n");
	}

	// method to present user's saved attributes in combo boxes for witch characters
	static void displayAttributesW(int index) 
	{
		String gender = (String)(witches[index].getGender()); // get gender of vampire character, cast to
		// string, store in variable
		if (gender.equals("Male"))      // if character is male
		{
			genderComboBox.setSelectedIndex(1); // default select male in combo box
		} 
	...
	{
//		System.out.println("deleteCharacter method, index vamp before delete: " + indexVamp);
//		System.out.println("charType: " + charType);
		...
		{
//			System.out.println("entered first if statement in deleteCharacter method");
...
//			System.out.println("index vamp after delete: " + indexVamp);
//			System.out.println("charListComboList count: " + clc);
		}
